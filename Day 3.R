...
steps1 = treat_instruction(input1) %>% all_positions() %>% as.data.frame
steps1$XY = paste(steps1$X, steps1$Y, sep = ", ")
steps1 = steps1[-1, ]
steps1$step1 = as.numeric(row.names(steps1)) - 1

steps2 = treat_instruction(input2) %>% all_positions() %>% as.data.frame
steps2$XY = paste(steps2$X, steps2$Y, sep = ", ")
steps2 = steps2[-1, ]
steps2$step2 = as.numeric(row.names(steps2)) - 1

result = steps1[which(steps1$XY %in% steps2$XY), ] %>%
  left_join(steps2[which(steps2$XY %in% steps1$XY), ])

result$sum = result$step1 + result$step2
